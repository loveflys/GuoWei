...
<html>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body" >
                        <form class="form-horizontal"  id="editForm" action="<%=path%>/company/update" method="post">
                            ...
                            <div class="form-group">
                              <label for="inputName" class="col-sm-3 control-label"><sp:message code="company.sectemplateId"/></label>
                              <div class="col-sm-9">
                                  <select class="form-control" name="sectemplateId" id="editsectemplate_container"  onchange="changesecTemplate(this.options[this.options.selectedIndex].value)">
                    ...
                    <div class="modal-body" >
                        <form class="form-horizontal"  id="addForm" action="<%=path%>/company/add" method="post">
                            ...
                            <div class="form-group">
                              <label for="inputName" class="col-sm-3 control-label"><sp:message code="company.sectemplateId"/></label>
                              <div class="col-sm-9">
                              <select class="form-control" name="sectemplateId" id="addsectemplate_container"  onchange="changesecTemplate(this.options[this.options.selectedIndex].value)">
    ...
    <div class="modal fade" id="managerModal" tabindex="-1" role="dialog"
        ...
        <script type="template" id="managerBody_tpl">
            ...
            [[ for(var index=0; index< window.param.ProList.length; index++){ var it=window.param.ProList[index]; ]]
                [[ if (it.storageracks == item.id) ]]
                <tr>
                    <td>
                        <img src="{{it.proimage}}" width="50px" height="50px" />
                    </td>
                    <td>{{it.proname}}</td>
                    <td><span class="badge bg-red">￥{{it.proprice}}</span></td>
                    <td>{{it.stock}}</td>
                    <td>{{it.warnstock}}</td>
                    <td onclick="showCPStock({{it.companyId}}, {{it.id}}, {{it.pid}}, {{it.stock}})"><i class="fa fa-plus-square"></i></td>
                </tr>        
            [[ } ]]
                                                            </tbody>
                                                        </table>
        ...
        function getData() {
            ...
            $.ajax({
                ...
                success: function(resp) {
                    window.param.ProList = resp.data
                    ...
                    $.ajax({
                        ...
                        success: function(res) {
                            window.param.storages = res.data
                            _.templateSettings = {
                                evaluate    : /\[\[(.+?)\]\]/g,
                                interpolate : /\{\{(.+?)\}\}/g
                            };
            ...
            function getAddData() {
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        $("#addModal input[name=sectemplateId]").val(res.data[0].id);
                        $("#addsectemplate_container").html(_.template($("#sectemplate_tpl").html())({
                            "data": res.data,
                            "stid": window.param.sectemplateId
                        }));
                    }
                });
                $.ajax({
            ...
            function getEditData() {
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        $("#editModal input[name=sectemplateId]").val(res.data[0].id);
                        $("#editsectemplate_container").html(_.template($("#sectemplate_tpl").html())({
                            "data": res.data,
                            "stid": window.param.sectemplateId
                        }));
                    }
                ...
                function changesecTemplate(id) {
                    $("#addModal input[name=sectemplateId]").val(id);
                    $("#editModal input[name=sectemplateId]").val(id);
                    window.param.sectemplateId = id;
                }
        </script>
